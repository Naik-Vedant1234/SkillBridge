...
function RegisterForm() {
  ...
  const [formData, setFormData] = useState({
    ...
    email: "",
    password: "",
    role: defaultRole as "student" | "mentor" | "company",
  });
  const [isLoading, setIsLoading] = useState(false);
  const [error, setError] = useState("");

  const handleRoleChange = (role: "student" | "mentor" | "company") => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const authData = await apiClient.register(formData);
      login(authData);
  ...
  const handleGoogleSignup = () => {
    ...
    const googleAuthURL = getGoogleOAuthURL(formData.role);
